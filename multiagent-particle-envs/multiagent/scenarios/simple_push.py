...
def load_obj(filename):
    with open('obj/' + filename +'.pkl', 'rb') as handle:
        obj = pickle.load(handle)
    return obj

class Scenario(BaseScenario):
    def make_world(self, num_agents_inpt=2, num_adversaries_inpt=1, num_landmarks_inpt=2):
        world = World()
    ...
    def reset_world(self, world, load = False):
        if load:
            world.agents, world.landmarks = load_obj('sp_state')
        else:
            ...
            world.landmarks[goal.index].color = np.array([0.1, 0.1, 1.0])
            for i, agent in enumerate(world.agents):
                agent.goal_a = goal
                agent.color = np.array([1.0, 1.0, 0.0])
                if agent.adversary:
            ...
            save([world.agents, world.landmarks], 'sp_state')
